...
public class ThermiteAutocannonRoundItem extends FlakAutocannonRoundItem {
    ...
    @Override public EntityType<?> getEntityType(ItemStack stack) { return ModEntities.THERMITE_AUTOCANNON.get(); }
    @Override
    public AbstractAutocannonProjectile getAutocannonProjectile(ItemStack stack, Level level) {
        AbstractAutocannonProjectile projectile = super.getAutocannonProjectile(stack, level);
        if (projectile instanceof ThermiteAutocannonProjectile p) {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        ...
        if (stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).contains("soul_fire"))
